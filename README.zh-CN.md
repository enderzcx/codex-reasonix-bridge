...
- **Native runtime**：`crb delegate` / `crb review` 通过 `reasonix delegate` 运行，不再依赖 `reasonix run`
- **30 秒快速开始**：安装、dry-run 测试、首次调用
- **纯商量入口**：`crb consult` / `crb ask` 用于 Codex 和 Reasonix 单纯讨论工程问题，不强制 review schema
